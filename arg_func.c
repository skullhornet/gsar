 ...
 *
 * In order to reset the scanner, call GetOpt with a NULL string as
 * the option string.
 */
int GetOpt(int argc, char **argv, const char *pOptStr)
{
    ...
    else
    {
        ...
        if (*pCurOpt != '\0')   /* remainder of argv is argument -d123 -> arg = 123 */
            pOptArg = pCurOpt;
        else if (*(++pOptChar) == ':') /* '::' optional arg, but no argument */
            pOptArg = NULL;
